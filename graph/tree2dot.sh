...
# to a image discribed by DOT language(provided by Graphviz)
...
filterstr="printf fopen fclose feof fscanf exit fread fprintf sprintf";
...
printf("\trankdir=LR;\n");
printf("\tsize=\"50,10\"\n");
printf("\tnode [fontsize=9,fontcolor=red,style=filled,fillcolor=lightblue]\n");
sort="sort -u"
}{
...
printf("\t\"%s\" -> \"%s\";\n", nodep[nodedepth-1], node) | sort ;
# printf("\t\"%s\" -> \"%s\"[label=\"%s>%s\"];\n", nodep[nodedepth-1], node);
}
}
